...
- **STUBS still to build** (all render `StubPage.jsx`): Company Deep Dive (`/companies`), GRC & Risk View (`/grc`), Recommendations (`/recommendations`). The CSV's long note/summary/recommendation text fields are so far unused — they're the natural material for Deep Dive and Recommendations.
