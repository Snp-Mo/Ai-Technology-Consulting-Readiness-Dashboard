...
- **Recommendations (`/recommendations`, `src/pages/Recommendations.jsx`) — DONE.** Automation opportunities + AI recommendation feed; the one page where Implementation Complexity is in scope (blueprint's Recommendations spec — GRC excludes it by design). `metrics.js` gained `isRecommendationReviewed(c)` (word-boundary `/\breviewed\b/i` test on `aiOutputReviewStatus` — see Key decisions) and `recommendationPriorities(companies)` (`{quickWins, longerTerm}` split of the opportunity-5 companies by complexity ≤3 / ≥4). Components in **`src/components/recommendations/`**:
  - `RecKpiRow.jsx` — Reviewed Recommendations (`44 / 50` on the full set; good when all reviewed, warn otherwise), Avg Automation Opportunity (/5, `scoreTier` color), Quick-Win Companies (good if >0).
  - `OpportunityLeaderboard.jsx` — top-10 table ranked by `automationOpportunityScore` desc, ties broken by lower complexity then higher `score0to100` (scores are heavily tied — whole dataset is 4–5); caption notes the skew; "Top 10 of N" caption when truncated.
  - `PrioritySplit.jsx` — "Quick wins" (good accent) vs "Longer-term bets" (warn accent) cards, company chips with mono `cx N` complexity values, per-card empty text.
  - `RecommendationFeed.jsx` — one sub-card per filtered company (industry-colored left border) with the full AI Recommendation text, or the italic **"Needs review — recommendation not yet reviewed"** fallback (same visual pattern as GRC's needs-review cards) when `isRecommendationReviewed` is false or the recommendation is blank — the 6 healthcare rows below surface here.
  `RecommendationsSkeleton` added to `Skeleton.jsx`. **All 5 core dashboard pages are now complete**; `StubPage.jsx` is no longer routed (kept in `src/pages/`).
...
- **Quick-win rule** (Recommendations page): `automationOpportunityScore === 5 && implementationComplexityScore <= 3`. The originally proposed `<= 2` threshold matches zero companies — the dataset's complexity floor is 3 (distribution: 3→8, 4→27, 5→15) — so quick wins are the lowest band that exists; currently 5 companies (Microsoft, Google, Amazon, Meta, Nvidia). "Longer-term bets" = opportunity 5 with complexity ≥4 (32 companies); opportunity-4 companies join neither list, consistent with the High Opportunity `=== 5` rule.
- **"Reviewed" recommendation heuristic**: `aiOutputReviewStatus` is long free text, never a literal `"Reviewed"`. `isRecommendationReviewed` tests `/\breviewed\b/i`: 44 rows say the row's outputs "have been reviewed"; the 6 that fail (Kaiser, UnitedHealth, Providence, HCA, Elevance, CommonSpirit — all Healthcare) describe the company's own AI-review *process* instead and never confirm the row itself was reviewed, so their recommendation cards show the needs-review fallback.
